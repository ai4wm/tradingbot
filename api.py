...
# 주문 TR 제한. 문서는 "1초당 5회"라고만 적혀 있으나 2026-09-03 실측에서
# 버스트 천장은 8~10건이었다. 실주문 취소 10건 동시 전송이 거부 0으로
# 통과했고(162300), 12·15·20건을 던져도 통과는 늘 10건 근처였다.
# 조회 TR과는 창이 분리된다(주문 5 + 조회 5 동시 = 거부 0). 매도(kt10001)와
# 취소(kt10003)도 서로 다른 버킷이다. 취소 9건 + 매도 1건 동시를 세 번
# 반복해 거부가 한 번도 없었고, 실계좌 예행연습(023790: 하한가 미체결 9건
# + 보유 1주)에서 10건이 모두 같은 순간에 나가 94ms에 끝났다. 그래서 취소
# 상한을 ORDER_BURST-1로 잡아도 매도가 창 밖으로 밀리지 않는다.
# 회복은 한 번에 풀리지 않고 건당 돌아온다. 10건을 채운 뒤 다음 1건이
# 통과하기까지 0.14~0.47초였고, 0.4초 뒤 5건을 한꺼번에 재전송하면 전부
# 튕겼다. 그래서 재전송 간격은 0.5초부터 잡는다.
#
# 10으로 잡는 이유는 상한가 붕괴 순간의 최대 묶음이 9분할 취소 + 전량매도
# = 10건이기 때문이다. 5로 두면 그중 5건만 나가고 나머지와 매도가 0.9초를
# 기다린다(2026-09-03 09:49 엠아이큐브솔루션: 그 사이 2회차 배분이 들어와
# 4건이 20주씩 더 체결됐고 매도는 0.73초 늦었다).
# 건마다 간격을 벌리면 취소 직후 매도가 그만큼 밀리므로, 최근 1초에
# ORDER_BURST건이 찼을 때만 기다리는 슬라이딩 창으로 센다.
