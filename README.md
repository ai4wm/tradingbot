...
- 분할매수, 매수잔량 단계별 자동매도(3단매도: 경고음·나머지취소·부분매도·전량매도), 100주 체결 뒤 잔량 자동취소, 비상청산
- 3단매도·자동취소·주문상태를 그날 안에서 재실행 복원(버튼을 눌러야 감시 시작)
- 뉴스 제목으로 테마를 붙이고, 사전을 고친 뒤 지난 기사에 소급 적용
- LS증권 실시간 뉴스 및 누락 뉴스 서버 동기화
- 네이버 관심종목 뉴스와 종목토론실
- 텔레그램 채널 뉴스 소급·실시간 수집과 앱 안 원문 보기
- 뉴스 상단 전광판(공급처별 색상 구분), 종목코드 유무별 알림음, 신규 글 강조
- 매수잔량 셀 클릭 시 종목별 확대 창(매수잔량·누적거래량 실시간, 항상 위, 위치·크기·투명도 저장)
- KRX 상한가 이력, DART 공시, 테마·수급·시장 지표 분석
- 상한가 후보 예측 및 워크포워드 검증
- 창별 레이아웃, 테마와 글꼴 크기 저장
- 분석창 시계 분리(반투명·가로세로 자유 크기 조절·위치와 크기 저장)
