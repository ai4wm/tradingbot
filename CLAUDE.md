...
- 뉴스 알림만 `rank.NEWS_ALERT_VOLUME`(기본 0.35)로 작게 냅니다. `winsound`는 음량 조절이 없어 뉴스일 때만 `QSoundEffect`로 재생하고, 실패하면 원래 경로로 돌아갑니다.
- `QSoundEffect`는 WAV를 비동기로 읽습니다. `Loading` 상태에서 `play()`하면 소리가 사라지므로 준비되면 한 번 재생하도록 예약합니다.
- 매수잔량 셀을 클릭하면 `gui.BidQtyPopup`이 종목별로 뜹니다. 타이틀바 없음, 항상 위, 드래그 이동, 우하단 크기 조절, 휠 투명도, 더블클릭 닫기, 조건 이탈 시 자동 닫기입니다.
- 확대 창 갱신은 타이머가 아니라 `0D` 호가 푸시가 직접 합니다. `bid_qty`가 실린 틱만 넘깁니다.
