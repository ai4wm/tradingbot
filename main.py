...
from PySide6.QtWidgets import (
    ...
    QPushButton, QSplitter, QTableWidget, QTextBrowser, QTableWidgetItem,
    QTabWidget, QVBoxLayout, QWidget, QInputDialog,
)
...
from analysis_db import (
    ...
    log_content_request, news_request_count_today, save_ls_realtime_news,
    ls_realtime_news_rows, search_ls_realtime_news,
    parse_ls_news_search_query, ls_realtime_news_detail,
    update_ls_realtime_news_source, update_ls_realtime_news_detail,
    update_ls_realtime_news_original_url, split_ls_news_stock_codes,
    ...
    save_telegram_news, telegram_news_rows,
)
from api import (
    KST, RestClient,
...
TELEGRAM_NEWS_VISIBLE_LIMIT = 500
...
class AnalysisWindow(LimitUpTabMixin, ThemeTabMixin, QMainWindow):
    ...
    def _insert_telegram_row(self, row: dict, highlight: bool):
        ...
        if highlight:
            ...
            body_item.setBackground(QColor(LS_NEWS_NEW_TITLE_BACKGROUND))
            body_item.setForeground(QColor(LS_NEWS_NEW_TITLE_FOREGROUND))
            font = body_item.font()
            font.setBold(True)
            body_item.setFont(font)
        while table.rowCount() > TELEGRAM_NEWS_VISIBLE_LIMIT:
